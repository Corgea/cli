...
//! Without a lockfile it execs behind an honest note. Without a token it uses
//! public fail-open vulnerability checks.
...
#[test]
fn uv_sync_tokenless_public_mode_checks_lockfile() {
    let mut h = SyncHarness::new(HashMap::new()).with_uv_lock(UV_LOCK);
