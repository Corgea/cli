...
use common::{key, npm_harness, pip_harness, vulnerable_body};
...
// npm parity for the verdict behaviors above (see `npm_harness`): the
// wildcard npm registry publishes every package in 2020, so recency never
// blocks — each block here is the vuln-api verdict's doing.

#[test]
fn npm_vulnerable_pin_blocks_without_running_install() {
    let mut checks = HashMap::new();
    checks.insert(
        key("npm", "axios", "1.0.0"),
        vulnerable_body("npm", "axios", "1.0.0", "GHSA-NPM-0001", Some("2.0.0")),
    );
    let mut h = npm_harness(checks, HashMap::new(), 0);
    let out = h
        .cmd
        .args(["npm", "install", "axios@1.0.0"])
        .output()
        .expect("run corgea");
    assert_eq!(out.status.code(), Some(1));
    assert_eq!(
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(stdout.contains("GHSA-NPM-0001"), "stdout: {stdout}");
    assert!(stdout.contains("critical"), "stdout: {stdout}");
    assert!(
        String::from_utf8_lossy(&out.stderr).contains("--force"),
        "block message must name --force"
...
#[test]
fn npm_force_overrides_vulnerable_block_and_propagates_exit_code() {
    let mut checks = HashMap::new();
    checks.insert(
        key("npm", "axios", "1.0.0"),
        vulnerable_body("npm", "axios", "1.0.0", "GHSA-NPM-0001", Some("2.0.0")),
    );
    let mut h = npm_harness(checks, HashMap::new(), 7);
    let out = h
        .cmd
        .args(["npm", "--force", "install", "axios@1.0.0"])
        .output()
        .expect("run corgea");
    assert_eq!(
        out.status.code(),
    ...
    assert_eq!(h.recorded_argv().as_deref(), Some("install axios@1.0.0"));
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("GHSA-NPM-0001"),
        "findings must still print under --force: {stdout}"
    );
}

#[test]
fn npm_no_fail_does_not_waive_vulnerable_block() {
    let mut checks = HashMap::new();
    checks.insert(
        key("npm", "axios", "1.0.0"),
        vulnerable_body("npm", "axios", "1.0.0", "GHSA-NPM-0001", Some("2.0.0")),
    ...
    let out = h
        .cmd
        .args(["npm", "--no-fail", "install", "axios@1.0.0"])
        .output()
        .expect("run corgea");
    assert_eq!(
        out.status.code(),
...
#[test]
fn npm_verdict_503_warns_and_fails_open() {
    let mut checks = HashMap::new();
    checks.insert(key("npm", "axios", "1.0.0"), "{}".to_string());
    let mut statuses = HashMap::new();
    statuses.insert(key("npm", "axios", "1.0.0"), 503u16);
    let mut h = npm_harness(checks, statuses, 0);
    let out = h
        .cmd
        .args(["npm", "install", "axios@1.0.0"])
        .output()
...
#[test]
fn npm_vuln_api_outage_warns_but_installs() {
    let mut h = npm_harness(HashMap::new(), HashMap::new(), 0);
    // Point the gate at a dead vuln-api: connection refused on every check.
    h.cmd.env("CORGEA_VULN_API_URL", "http://127.0.0.1:1");
    let out = h
        .cmd
