...
#[test]
fn uv_add_requirements_file_is_tree_gated() {
    let cwd = TempDir::new().expect("temp cwd");
    std::fs::write(cwd.path().join("requirements.txt"), "oldpkg==1.0.0\n")
        .expect("write requirements.txt");
    let mut checks = HashMap::new();
    ...
    let out = h
        .cmd
        .current_dir(cwd.path())
        .args(["uv", "add", "-r", "requirements.txt"])
        .output()
        .expect("run corgea");
    assert_eq!(out.status.code(), Some(1), "transitive vuln must block");
    assert_eq!(h.recorded_argv(), None, "uv must not run when blocked");
...
#[test]
fn uv_pip_sync_requirements_file_is_tree_gated() {
    let cwd = TempDir::new().expect("temp cwd");
    std::fs::write(cwd.path().join("requirements.txt"), "oldpkg==1.0.0\n")
        .expect("write requirements.txt");
    let mut checks = HashMap::new();
    ...
    let out = h
        .cmd
        .current_dir(cwd.path())
        .args(["uv", "pip", "sync", "requirements.txt"])
        .output()
        .expect("run corgea");
    assert_eq!(out.status.code(), Some(1), "transitive vuln must block");
    assert_eq!(h.recorded_argv(), None, "uv must not run when blocked");
