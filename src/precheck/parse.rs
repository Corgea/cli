...
pub struct ParsedInstall {
    ...
    /// `pip install -r foo.txt` — the requirements files we should
    /// load and verify in lieu of standalone targets.
    pub requirements_files: Vec<PathBuf>,
    /// True if the user invoked the bare install (`npm install` /
    /// `pip install` with no positional specs and no `-r`).
...
mod tests {
    ...
    fn npm_kind(t: &InstallTarget) -> &TargetKind {
        &t.kind
    }
    ...
    fn parse_npm_spec_classifies() {
        ...
        for (input, expected) in cases {
            ...
            match (npm_kind(&target), &expected) {
