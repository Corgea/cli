...
pub fn parse_pypi_positionals_args(args: &[String]) -> ParsedInstall {
    build_parsed_install(extract_uv_add_positionals(args), parse_pypi_spec)
}
...
pub fn classify_uv_command(cmd: &[String]) -> UvCommand<'_> {
    match cmd.first().map(String::as_str) {
        ...
        Some("pip") if matches!(cmd.get(1).map(String::as_str), Some("sync")) => {
            UvCommand::PipSync {
                sync_args: &cmd[2..],
            }
        }
        ...
        Some("install") => UvCommand::InvalidInstall,
        _ => UvCommand::Passthrough,
    }
}

pub fn parse_uv_pip_sync_args(args: &[String]) -> ParsedInstall {
    let positionals = extract_node_positionals(PackageManager::Uv, args);
    ParsedInstall {
        targets: Vec::new(),
        requirements_files: positionals.specs.into_iter().map(PathBuf::from).collect(),
    }
}
...
fn extract_uv_add_positionals(args: &[String]) -> PositionalSplit {
    let mut out = PositionalSplit::default();
    let mut i = 0;
    while i < args.len() {
        let a = &args[i];
        ...
        if a.starts_with('-') {
            if !a.contains('=') && takes_value(PackageManager::Uv, a) {
                i += 2;
            } else {
                i = skip_unknown_flag(args, i);
            }
