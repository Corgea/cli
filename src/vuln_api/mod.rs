...
pub fn check_package_version(
    ...
    check_package_version_with_auth(client, base_url, Some(token), ecosystem, name, version)
}

pub fn check_package_version_with_auth(
    client: &reqwest::blocking::Client,
    base_url: &str,
    token: Option<&str>,
    ecosystem: &str,
    ...
    match status.as_u16() {
        401 => {
            let msg = if token.is_some() {
                "vuln-api rejected the Corgea token (run `corgea login` to refresh)"
            } else {
                "vuln-api rejected the public unauthenticated request"
            };
            return Err(msg.into());
        }
        403 => {
