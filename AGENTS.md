...
- Most CLI commands (`scan`, `list`, `inspect`, `wait`) require authentication via `CORGEA_TOKEN` or `corgea login`. Without a token, commands exit with a clear "No token set" error — this is expected behavior, not a build issue.
